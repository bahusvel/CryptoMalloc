...
typedef struct procstat {
	...
	/** 1 **/				  /** R is running, S is sleeping,
											  D is sleeping in an header wait,
	...
	unsigned int itrealvalue; /** The time before the next SIGALRM is sent to
								 the process **/
...
int get_proc_info(procstat *pinfo) {
	char szFileName[PATH_MAX], szStatStr[2048], *s, *t;
	FILE *fp;
	struct stat st;
	...
	sprintf(szFileName, "/proc/self/stat");
	...
	sscanf(t + 2, "%c %d %d %d %d %d %u %u %u %u %u %d %d %d %d %d %d %u %u %d "
				  "%u %u %u %u %u %u %u %u %d %d %d %d %u",
		   ...
		   &(pinfo->counter), &(pinfo->priority), &(pinfo->timeout),
		   &(pinfo->itrealvalue), &(pinfo->starttime), &(pinfo->vsize),
		   &(pinfo->rss), &(pinfo->rlim), &(pinfo->startcode),
